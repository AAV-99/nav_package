...
class RD03D:
    ...
    def __init__(self, uart_port='/dev/ttyTHS0', baudrate=256000, multi_mode=True):
        """
        Initialize RD03D radar for Jetson Orin Nano
        
        Args:
            uart_port: UART device path. Common options:
                      - '/dev/ttyTHS0' (UART1 on 40-pin header)
                      - '/dev/ttyTHS1' (UART2 - Debug)
            baudrate: Communication speed (default 256000)
            multi_mode: True for multi-target mode, False for single-target
        """
        try:
            ...
            print(f"RD03D radar initialized on {uart_port}")
        except serial.SerialException as e:
            print(f"Error opening UART port {uart_port}: {e}")
            print("Make sure UART is enabled and you have proper permissions.")
    ...
    def get_all_targets(self):
        """Get all detected targets as a list."""
        return self.targets
    ...
    def close(self):
        ...
        if self.uart.is_open:
            self.uart.close()
            print("RD03D radar connection closed")